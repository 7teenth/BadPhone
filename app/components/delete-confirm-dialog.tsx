...
import { AlertTriangle, Package, DollarSign, Hash, Calendar, Trash2, Shield, Info, X } from "lucide-react"
...
export function DeleteConfirmDialog({
  ...
  const getImpactWarnings = () => {
    ...
    if (totalValue > 1000) {
      warnings.push({
        type: "financial",
        message: `Загальна вартість товару на складі: ${totalValue.toLocaleString()} ₴`,
        severity: "high",
      })
    }
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="bg-gray-50 border-l-4 border-red-500 p-4 rounded-r-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {product.description && <p className="text-sm text-gray-600 mt-2 italic">"{product.description}"</p>}
              </div>
            </div>
          </div>
          ...
          {impactWarnings.length > 0 && (
            <div className="space-y-3">
              ...
              {impactWarnings.map((warning, index) => (
                ...
                >
                  <AlertTriangle
                    className={`h-4 w-4 ${warning.severity === "high" ? "text-red-600" : "text-yellow-600"}`}
                  />
                  <AlertDescription className={warning.severity === "high" ? "text-red-800" : "text-yellow-800"}>
                    {warning.message}
                  </AlertDescription>
                </Alert>
              ))}
              ...
              <div className="mt-3 p-4 bg-gray-50 rounded-lg text-sm space-y-2">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="ml-2">{formatDate(product.updated_at)}</span>
                  </div>
                </div>
                {(product as any).purchasePrice && (
                  <div>
                    <span className="font-medium">Ціна закупки:</span>
                    <span className="ml-2">{(product as any).purchasePrice} ₴</span>
                    <span className="ml-2 text-gray-600">
                      (прибуток: {(product.price - (product as any).purchasePrice).toFixed(2)} ₴)
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-4 border-t pt-4">
            <h4 className="font-medium text-gray-900">Для підтвердження видалення:</h4>
...
/>
              <Label htmlFor="confirm-checkbox" className="text-sm leading-relaxed">
                Я розумію, що це дія незворотна і товар буде повністю видалений з системи
              </Label>
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="confirm-text">
                Введіть <code className="bg-gray-200 px-1 rounded font-mono">{expectedConfirmText}</code> для
                підтвердження:
              </Label>
              <Input
                id="confirm-text"
          ...
          <div className="text-xs text-gray-500 text-center pt-2 border-t">
            ...
            <kbd className="bg-gray-100 px-1 rounded ml-1">Esc</kbd> - скасувати
          </div>
        </CardContent>
      </Card>
    </div>
