...
import { ArrowLeft, Plus, Search, Package, Edit, Trash2, AlertCircle, Filter } from "lucide-react"
...
export function ProductCatalog({ onBack }: ProductCatalogProps) {
  const { products, stores, currentUser, addProduct, updateProduct, deleteProduct, isOnline } = useApp()
          ...
          <TabsContent value="list" className="space-y-4">
            {filteredProducts.length === 0 ? (
              <Card className="p-12 text-center">
                ...
                <p className="text-gray-500">Спробуйте змінити критерії пошуку або додайте новий товар</p>
              </Card>
            ) : (
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 text-sm">
                              ...
                              <span className="text-gray-600">Кількість: {product.quantity} шт</span>
                              {product.barcode && <span className="text-gray-600">Штрих-код: {product.barcode}</span>}
                              {store && <span className="text-gray-600">Магазин: {store.name}</span>}
                            </div>
                          </div>
                      ...
                      <CardContent className="p-4">
                        <div className="space-y-3">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="text-xs text-gray-600">Кількість: {product.quantity} шт</div>
                            {store && <div className="text-xs text-gray-600">Магазин: {store.name}</div>}
                          </div>
